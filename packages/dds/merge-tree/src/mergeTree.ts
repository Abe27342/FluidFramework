		...
		for (const newSegment of newSegments
			...
			if (oldest && newest?.clientId !== clientId) {
				...
				if (newestAcked === newest || newestAcked?.clientId !== clientId) {
					moveInfo = {
						// This is the wrong move info when the newest obliterate is local + other clients think that the 'second newest' is actually allowed to insert.
						movedClientIds,
						movedSeq: oldest.seq,
						movedSeqs,
				...
				if (moveInfo.localMovedSeq !== undefined) {
					...
					// );
					assert(oldestUnacked?.segmentGroup !== undefined, "Expected segment group to exist");
